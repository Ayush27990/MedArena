...
async def stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db_user:
        await update.message.reply_text("❌ Could not retrieve your stats.")
        return
    ...
    progress = f"{xp}/{next_xp} XP to next rank" if next_xp else "Max rank achieved!"
...
async def leaderboard_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, row in enumerate(rows):
        ...
        lines.append(
            f"{medal} *{name}*\n"
            f"   ⭐ {row['xp']} XP | 🎯 {row['accuracy']}% | "
            f"📝 {row['total_attempted']} attempted"
        )
